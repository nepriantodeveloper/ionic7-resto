...
export class OrderDetailPage implements OnInit {
  ...
  async presentModal() {
    ...
    modal.onDidDismiss().then((selected) => {
      ...
      if (selected && selected.role == 'selected') {
        if (selected.data && selected.data.length) {
          this.util.show('Please wait');
          this.api.post_private('v1/orders/acceptOrder', { id: this.id, status: 'accepted', did: selected.data[0].id }).then((order: any) => {
            console.log(order);
            this.util.hide();
            if (order && order.status && order.status == 200 && order.data) {
              this.sendNotification(this.util.translate('New Order'), this.util.translate('New Order Received'), selected.data[0].fcm_token);
              const msg = this.util.translate('Your Order is') + ' ' + this.util.translate('accepted') + ' ' + this.util.translate('By') + ' ' + this.restName;
              this.sendNotification(msg, msg, this.token);
              Swal.fire({
                title: this.util.translate('success'),
