...
export class RegisterPage implements OnInit {
  ...
  onRegister(form: NgForm) {
    console.log(this.util.appSettings.user_verify_with);
    this.submitted = true;
    if (form.valid) {
      if (this.register.check == false) {
        this.util.showToast('Please accept terms and condition of apps', 'dark', 'bottom');
        return false;
      }
    ...
    this.api.post_private('v1/referral/redeemReferral', { id: localStorage.getItem('uid'), code: this.register.referral }).then((data: any) => {
      ...
      if (data && data.status && data.status == 200 && data.data) {
        ...
        if (data && data.data && data.data.who_received == 1) {
          ...
          text = this.util.translate('Congratulations you have received the') + ' ' + this.util.appSettings.currencySymbol + ' ' + data.data.amount + ' ' + this.util.translate('on wallet');
        } else if (data && data.data && data.data.who_received == 3) {
          text = this.util.translate('Congratulations you & your friend have received the') + ' ' + this.util.appSettings.currencySymbol + ' ' + data.data.amount + ' ' + this.util.translate('on wallet');
        }
