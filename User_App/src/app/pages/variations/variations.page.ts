...
export class VariationsPage implements OnInit {
  ...
  radioGroupChange(event: any, title: any) {
    console.log(this.lists);
    const radioList = this.lists.filter((x: any) => x.title == title);
    const selectedItems = radioList[0].items.filter((x: any) => x.title == event.detail.value);
  ...
  checkedEvent(event: any, title: any) {
