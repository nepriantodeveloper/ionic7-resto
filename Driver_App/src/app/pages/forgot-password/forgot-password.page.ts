...
export class ForgotPasswordPage implements OnInit {
  ...
  onLogin(form: NgForm) {
    ...
    if (form.valid) {
      ...
      this.api.post_public('v1/driver/verifyEmailForReset', this.login).then((data: any) => {
        this.isLogin = false;
        console.log(data);
  ...
  resetPassword(form: NgForm) {
    ...
    if (form.valid) {
      ...
      this.api.post_temp('v1/password/updateUserPasswordWithEmailDriver', param, this.temp).then((data: any) => {
        this.isLogin = false;
        console.log(data);
        if (data && data.status && data.status == 200 && data.data) {
  ...
  resetPasswordPhone(form: NgForm) {
    ...
    if (form.valid) {
      ...
      this.api.post_temp('v1/password/updateUserPasswordWithPhoneDriver', param, this.temp).then((data: any) => {
        this.isLogin = false;
        console.log(data);
        if (data && data.status && data.status == 200 && data.data) {
          this.util.showToast('Password Updated', 'success', 'bottom');
  ...
  resetPasswordPhoneFirebase(form: NgForm) {
    ...
    if (form.valid) {
      ...
      this.api.post_temp('v1/password/updateUserPasswordWithPhoneFirebaseDriver', param, this.temp).then((data: any) => {
        this.isLogin = false;
        console.log(data);
        if (data && data.status && data.status == 200 && data.data) {
          this.util.showToast('Password Updated', 'success', 'bottom');
  ...
  onOTPLogin(form: NgForm) {
    ...
    if (form.valid) {
      if (this.util.appSettings.sms_name == '2') {
        ...
        this.isLogin = true;
        this.api.post_public('v1/driver/verifyPhoneForFirebase', this.mobile).then((data: any) => {
          console.log(data);
          this.isLogin = false;
          if (data && data.status && data.status == 200 && data.data) {
        ...
        this.isLogin = true;
        this.api.post_public('v1/driver/verifyPhone', this.mobile).then((data: any) => {
          this.isLogin = false;
          console.log(data);
  ...
  generateFirebaseToken() {
    ...
    this.api.post_temp('v1/otp/getFirebaseTokenDriver', param, this.temp).then((data: any) => {
      this.isLogin = false;
      console.log(data);
